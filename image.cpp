...
void Image::draw(int x, int y, float scale, float angle) const {
  ...
  SDL_RenderCopyEx(renderer, texture, &view, &dest, static_cast<double>(angle),
                   NULL, SDL_FLIP_NONE);
}
...
Image* Image::crop(SDL_Rect sub) const {
  if(sub.x+sub.w > view.w || sub.y+sub.h > view.h){
    std::cerr 
      << "Attempted to crop image with invalid geometry."
      << std::endl
      << "(0,0 + " << view.w << "x" << view.h << ")"
      << " --> "
