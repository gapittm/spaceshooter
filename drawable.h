...
class Drawable {
  ...
  public:
    Drawable(const std::string& n, const Vector2f& pos, const Vector2f& vel,
      ...
      name(n), position(pos), velocity(vel), scale(1.0), angle(a),
      rotateSpd(r)
    { }

    Drawable(const Drawable& s) : 
      name(s.name), position(s.position), velocity(s.velocity),
      scale(s.scale), angle(s.angle), rotateSpd(s.rotateSpd)
    { }
    ...
    float getRotSpd() const  { return rotateSpd; }
    void  setRotSpd(float s) { rotateSpd = s; }
